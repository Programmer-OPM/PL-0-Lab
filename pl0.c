...
void getsym(void)
{
	...
	else
	{
		...
		else if (ch == ':')
		{
			...
			{
				sym = SYM_COLON;       // modified by zhf, required in 'for' statement
			}
		}
		else if (ch == '>')
		{
...
void statement(symset fsys)
{
	...
	else if (sym == SYM_GOTO) //goto的实现
	{
		...
		else
		{
			...
			{
				enter(ID_LABEL);
				gen(JMP, 0, -1); //先出现goto 后面才出现label暂时填-1,表示label未出现
			}
			else if (table[i].kind == ID_LABEL)
			{
				if (code[table[i].value].f == JMP && code[table[i].value].a == -1)
				{
					code[table[i].value].a = cx;
					table[i].value = cx;
					gen(JMP, 0, -1); //依然未出现label 但之前出现了GOTO,使上一个GOTO到下一个GOTO
				}
				else
	...
	{ // for statement
		int pn_flag = 1; //whether the last read number is negative
		getsym();
		if(sym == SYM_LPAREN)
		{
			getsym();
		...
		{
			error(28); //Missing number(s) in in 'for' statement
		}
		mask *mk;
		i = position(id);
		mk = (mask*) &table[i];
		gen(LIT, 0, num * pn_flag);
		gen(STO, level - mk->level, mk->address); //initialize control variable
		int up, step; //looping conditions
		if(sym == SYM_COMMA)
		{
			getsym();
		}
		...
		cx1 = cx;
		gen(LOD, level - mk->level, mk->address); //load loop variable
		gen(LIT, 0, up);
		gen(OPR, 0, OPR_NEQ);
		cx2 = cx;
		gen(JPC, 0, 0); //check looping conditions
		//statement starts here
		statement(fsys);
		gen(LOD, level - mk->level, mk->address);
		gen(LIT, 0, step);
		gen(OPR, 0, OPR_ADD);
		gen(STO, level - mk->level, mk->address); //update loop variable with step
		gen(JMP, 0, cx1);
		code[cx2].a = cx;
	}//for statement by zhf
	test(fsys, phi, 19);
} // statement
...
{									 //paranum 为当前程序体的参数个数
	...
	if(!in_procedure){
		int q;
		for(q=pre_cx-single_pre_cx;q<pre_cx;q++){
	...
	{
		int q;
		for(q=pre_cx_p-single_pre_cx_p;q<pre_cx_p;q++){
...
int main()
{
	...
	statbegsys = createset(SYM_BEGIN, SYM_CALL, SYM_IF, SYM_WHILE, SYM_FOR,  SYM_GOTO, SYM_PRT, SYM_NULL); //添加了GOTO by 徐卓
